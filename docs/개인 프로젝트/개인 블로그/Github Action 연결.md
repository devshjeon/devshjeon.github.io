...
그 이유는 파일을 다운로드하는 스크립트 실행 후 commit & push 된 내용이 빌드 시점에 반영이 안 되는 문제가 있기 때문이다. (한 번에 하는 방법을 찾았지만, 과정이 번거로워 비교적 간편한 2단계로 나누는 방법을 선택!)


단계별로 배포 스크립트가 하는 역할을 다음과 같다.


### 1단계 배포 스크립트

1. Notion에 있는 글을 가져와 마크다운 파일로 만드는 스크립트를 실행 후 Github에 commit & push
2. 2단계 배포 스크립트 실행
...
이번에는 기존에 즐겨 사용하고 있는 Notion에서 글을 작성하고 자동화를 통해 배포를 손쉽게 할 수 있어, 앞으로 주기적으로 포스팅하는 데 큰 도움이 될 것 같다.


많은 블로그와 공식 문서를 참고하면서 기존 문제를 해결해 나가는 과정이 즐거웠고, 앞으로 블로그에 기능이 추가될 때마다 계속 포스팅할 계획이다.

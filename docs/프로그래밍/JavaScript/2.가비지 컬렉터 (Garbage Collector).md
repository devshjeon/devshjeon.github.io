...
has_children: false
last_modified_date: 2023-08-30 15:32
nav_order: 2
grand_parent: 프로그래밍
parent: JavaScript
---
...
- 참조된다고 해서 도달 가능성이 있는 것은 아니다. 서로 연결된 객체들도 도달 불가능할 수 있다. (도달할 수 없는 섬)
...
도달 가능성이란 어떻게든 접근해서 사용할 수 있는 값을 의미하고, 도달 가능한 값은 메모리에서 삭제되지 않는다.


아래의 값은 도달 가능한 값으로 명백한 이유 없이는 삭제 되지 않는다.
...
위 family 예시에서 family 객체에 null을 덮어쓰면 아래와 같은 구조로 변경된다.
...
1. GC는 루트 정보를 수집하고 mark(기억)한다.
2. 루트가 참조하고 있는 모든 객체를 방문하고 이것도 mark한다.
3. mark된 모든 객체에 방문하고 그 객체들이 참조하는 객체도 mark한다. (한번 방문한 객체는 mark하기 때문에 같은 객체를 다시 방문하지 않는다)
...
이를 해결하기 위해 V8 엔진에서는 Orinoco 프로젝트를 통해 성능 최적화 방법을 발전시켰다.
...
메인 스레드와 헬퍼 스레드가 거의 동일한 양의 작업을 동시에 수행하는 방식이다.
...
메인 스레드가 자바스크립트를 지속적으로 실행하고 헬퍼 스레드는 GC 작업만 수행하는 방식이다.
